...
function mapProduct(p) {
    return {
      ...
      tags: p.tags || [],
      reviews: p.reviews || [],
      createdAt: p.meta?.createdAt || new Date(),
      updatedAt: p.meta?.updatedAt || new Date(),
    };
  }
...
for (const p of products) {
    const fullProduct = await fetchProductById(p.id); // fetch full details
    const doc = mapProduct(fullProduct);
